...
module.exports = class CacheVFS {
  ...
  // mask: raw bitmap (1 bit per page, LSB first); every set bit clears the
  // page's presence bit so the next read of it refetches via _miss. The
  // lazy counterpart to applyPages — same caller contract.
  invalidatePages(mask) {
    binding.cacheVfsInvalidate(this._handle, mask)
  }
}
